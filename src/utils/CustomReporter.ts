...
class CustomTTAReporter implements Reporter {
    ...
    // Strip ANSI color/style escape codes (e.g. from util.inspect / colorized
    // console.log output) so they don't leak into the HTML report as literal
    // "Ø[33m..." garbage.
    // eslint-disable-next-line no-control-regex
    private static readonly ANSI_PATTERN = /\x1b\[[0-9;]*m/g;

    private stripAnsi(text: string): string {
        return text.replace(CustomTTAReporter.ANSI_PATTERN, '');
    }
    ...
    private extractTestLogs(result: TestResult): string[] {
        ...
        for (const chunk of result.stdout) {
            const text = this.stripAnsi(typeof chunk === 'string' ? chunk : chunk.toString());
        ...
        for (const attachment of result.attachments) {
            ...
            if (logMatch && attachment.contentType === 'text/plain') {
                ...
                if (stepIndex >= 0 && stepIndex < testSteps.length) {
                    ...
                    if (logContent) {
                        const logs = this.stripAnsi(logContent).split('\n').filter((line: string) => line.trim());
                        if (logs.length > 0) {
